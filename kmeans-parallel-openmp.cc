...
float simd_dist(vector<float> b,vector<float> b1, int N)
{

	int alignedN = N - N % 4;
	for (int i = 0; i < alignedN; i+=4) {
	_mm_storeu_ps(&b[i], 
					_mm_add_ps(_mm_loadu_ps(&b[i]), 
					-_mm_loadu_ps(&b1[i])));

	_mm_storeu_ps(&b[i], 
					_mm_mul_ps(_mm_loadu_ps(&b[i]), 
					_mm_loadu_ps(&b[i])));
...
private:
	...
	int total_values, total_points, max_iterations,n_threads,grain_size,simd;
	...
	int getIDNearestCenter(Point point)
	{
		...
		double min_dist=0;
		 ...
		 //int N=point.getTotalValues();

		 vector<double> d=point.getValues();
		 std::vector<float> b(d.begin(), d.end());

		if (simd==1)
		{
			
			vector<double> d1=clusters[0].getValues();
			std::vector<float> b1(d1.begin(), d1.end());
			min_dist=simd_dist(b,b1,total_values); 
		} 
		...
		else
		{
				for(int i = 0; i < total_values; i++)
			{
				min_dist += pow(clusters[0].getCentralValue(i) -
		...
		for(int i = 1; i < K; i++)
		{
			...
			if (simd==1)
			{
				vector<double> d1=clusters[i].getValues();
				std::vector<float> b1(d1.begin(), d1.end());
				dist=simd_dist(b,b1,total_values); 
			}
...
public:
	KMeans(int K, int total_points, int total_values, int max_iterations,int n_threads, int grain_size, int simd)
	{
		this->K = K;
...
int main(int argc, char *argv[])
{
  ...
  int o;
  while ((o = getopt(argc, argv, "t:g:s:d:h")) != -1) {
    switch (o) {
    case 't':
	...
	KMeans kmeans(K, total_points, total_values, max_iterations,NThreads,grain_size,simd);
	kmeans.run(points);
